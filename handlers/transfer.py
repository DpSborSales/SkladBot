...
def register_transfer_handlers(bot):
    ...
    def select_product(call):
        ...
        for v in transfer_variants:
            markup.add(types.InlineKeyboardButton(
                v['name'],
                callback_data=f"transfer_var_{v['id']}"
            ))
    ...
    def select_variant(call):
        ...
        bot.register_next_step_handler_by_chat_id(call.message.chat.id, process_transfer_quantity, user_id)
        bot.answer_callback_query(call.id)

    def process_transfer_quantity(message, user_id):
        ...
        if hub_seller:
            ...
            try:
                bot.send_message(
                    ...
                    f"От: {seller['name']}\n"
                    f"Товар: {product_name} ({variant['name']})\n"
    ...
    def approve_transfer(call):
        ...
        if req['status'] != 'pending':
            ...
            return
        try:
            # Списать со склада хаба (HUB_SELLER_ID) по данному варианту
